...
```{r data-setup, include=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
lags_behind_italy <- c(0, 19, 11, 19)
names(lags_behind_italy) <- c("Italy", "California", "New York", "Washington")

by_country_state %>%
  filter(country %in% names(lags_behind_italy)) %>%
  mutate("day" = date - min(date),
         "lag_date" = date - days(lags_behind_italy[country]),
         "lag_days" = lag_date - min(date)) %>%
