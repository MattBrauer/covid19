...
```{r ba-county-plot, fig.wide = TRUE, fig.cap = "Cases and deaths within California.", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r per-capita-ba-county-plot, fig.wide = TRUE, fig.cap = "Cases and deaths per million within California.", echo=FALSE, warning=FALSE, message=FALSE}
...
exp_fit <- function(dataset, political_unit, variable, before) {
  dataset %>%
...
exp_fits <- by_country_state %>%
  filter(country %in% countries, cases > 9) %>%
  exp_fit(quo(country), quo(cases), before = 5) %>%
  group_by(country) %>%
...
exp_fits <- us_states %>%
  filter(cases > 9) %>%
  exp_fit(quo(state), quo(cases), before = 5) %>%
  group_by(state) %>%
  mutate(max_est = purrr::map_dbl(data, ~max(.$estimate)),
         max_date = as.Date(purrr::map_dbl(data, ~ .$date[which.max(.$estimate)]),
                            origin = "1970-01-01"),
