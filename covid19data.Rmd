...
```{r by-county, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}

complete %>%
  filter(country=="US", state=="California", !is.na(Admin2)) %>%
  group_by(Admin2) %>%
ggplot(aes(date, deaths, group=Admin2)) +
  geom_line(aes(color=Admin2)) +
  geom_text(aes(label = Admin2, color = Admin2, x = date, y = deaths), hjust = -.5) +
  theme_minimal() +
  theme(legend.position = "none")
