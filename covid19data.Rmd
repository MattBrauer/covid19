...
```{r plot-all-states, fig.wide = TRUE, fig.cap = "Progression of cases between countries. Day 0 for each country is the first day for which there is data. In some cases (e.g., China, Japan) the pandemic was well underway before data were recorded by JHU.", echo=FALSE, warning=FALSE, message=FALSE}
...
p1 <- states_current_barplot(us_states_current, quo(cases))
p2 <- states_current_barplot(us_states_current, quo(deaths))

grid.arrange(p1, p2, nrow = 1)

```

```{r states-per-capita, echo=FALSE, warning=FALSE, message=FALSE}

us_states_per_capita <- us_states_current %>%
  left_join(us_county_pop %>% filter(COUNTY == "000"), by = c("state" = "STNAME")) %>%
  filter(!is.na(pop)) %>%
  dplyr::select(state, pop, date, cases, recovered, deaths) %>%
  mutate_at(c("cases", "recovered", "deaths"), ~ (. * 1e6)/pop)
```


```{r states-per-capita-map-plot, fig.wide = TRUE, fig.cap = "Cases and deaths per million inhabitants, by state.", echo=FALSE, warning=FALSE, message=FALSE}

p1 <- us_states_per_capita %>% state_map(quo(cases))
p2 <- us_states_per_capita %>% state_map(quo(deaths))

grid.arrange(p1, p2, nrow = 1)

```

```{r states-per-capita-bar-plot, fig.wide = TRUE, fig.cap = "Cases and deaths per million inhabitants, by state. Ten most affected states shown.", echo=FALSE, warning=FALSE, message=FALSE}

p1 <- us_states_per_capita %>% states_current_barplot(quo(cases))
p2 <- us_states_per_capita %>% states_current_barplot(quo(deaths))
...
```{r curve-fits, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
exp_phase_fit_cases <- exp_phase_fit(by_country_state, bounds, quo(cases))
exp_phase_fit_deaths <- exp_phase_fit(by_country_state, bounds, quo(deaths))
