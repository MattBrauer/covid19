...
```{r daily-from-cumulative, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r state-daily, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
state_log_plot <- function(dataset, variable, topn=5) {
  dataset %>%
   ...
   geom_text(data = subset(., day == maxday), aes(label = state,
    ...
    #             ungroup() %>%
    #             top_n(topn, (!! variable)),
    #           aes(label = state, color = state, x = day, y = (!! variable)),
    #           hjust = "right",
    #           vjust = "bottom") +
    xlab("Day of pandemic") +
    ylab(quo_name(variable)) +
