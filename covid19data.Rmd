...
```{r plot-all-states, fig.wide = TRUE, fig.cap = "Progression of cases between states. Day 0 for each country is the first day for which there is data. In some cases (e.g., China, Japan) the pandemic was well underway before data were recorded by JHU.", echo=FALSE, warning=FALSE, message=FALSE}
...
state_log_plot <- function(dataset, variable) {
  dataset %>%
    filter(!!variable >= 10) %>%
    group_by(state) %>%
    mutate(day = date - min(date), maxday = max(day)) %>%
...
county_map <- function(dataset, variable) {
  log_breaks <- 2 * 5**seq(0,8)
  counties_map %>%
...
```
```{r cper-capita-ca-county-plot, fig.wide = TRUE, fig.cap = "Cases and deaths per million within California.", echo=FALSE, warning=FALSE, message=FALSE}
...
#     unnest(bounds) %>%
#     ungroup()
#   exp_start <- bounds_var %>% select(country, min) %>% deframe()
#   exp_end <- bounds_var %>% select(country, max) %>% deframe()
#   dataset %>%
#   filter(country %in% bounds_var$country,
...
countries <- c("China","New York","Italy","Spain", "California","Washington", "US")
countries <- c("New York","Italy","Spain", "California","Washington", "US")

exp_fits <- by_country_state %>%
  filter(country %in% countries) %>%
  exp_fit(quo(cases), before = 5) %>%
  group_by(country) %>%
...
exp_fits %>%
  select(country, max_est, max_date) %>%
