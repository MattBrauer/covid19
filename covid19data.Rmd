...
```{r state-map-plot, echo=FALSE, warning=FALSE, message=FALSE}
...
state_map(quo(confirmed))

```

```{r county-map-plot, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}

county_map <- function(state, variable) {
  states_map %>%
    left_join(
      us_states_current %>%
        mutate(fips = state_abbreviations[state],
...
plot_counties <- function(plot_country, plot_state, variable, topn=3) {
  complete %>%
    filter(country==plot_country, state==plot_state, !is.na(Admin2)) %>%
    group_by(Admin2) %>%
  { ggplot(.) +
    geom_line(aes(x=date, y=(!!variable), color=Admin2)) +
    geom_text(data = . %>%
                filter(date == max(date)) %>%
                ungroup() %>%
                top_n(topn, (!! variable)),
              aes(label = Admin2, color = Admin2, x = date, y = (!! variable)),
              hjust = "right",
              vjust = "bottom") +
      theme_minimal() +
      theme(legend.position = "none") +
...
plot_counties("US", "Michigan", quo(confirmed))

plot_counties("US", "California", quo(deaths))
plot_counties("US", "California", quo(confirmed))
