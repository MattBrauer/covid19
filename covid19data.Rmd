...
lag_days_new <- c("China"=0,"Italy"=31,"Spain"=36,"US"=40,"California"=57,"New York"=9,"Washington"=53)

by_country_state %>%
  filter(country %in% countries) %>%
  mutate("day" = date - min(date),
         "lag_date" = date - days(lag_days_new[country] - lag_days_new["Italy"]),
         "lag_days" = lag_date - min(date)) %>%
...
exp_start_deaths <- as.Date(c("2020-01-22", "2020-03-02", rep("2020-03-01", 5)))
...
by_country_state %>%
  ...
  mutate("day" = date - lag_days[country], "log_confirmed" = log(confirmed + 1)) %>%
...
by_country_state %>%
  filter(country %in% c(exp_phase_countries, "US")) %>%
  mutate("day" = date - lag_days[country], "log_deaths" = log(deaths + 1)) %>%
...
#  mutate("day" = day - lag_days[country]) %>%
