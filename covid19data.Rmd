...
```{r county-map-plot, echo=FALSE, warning=FALSE, message=FALSE}
...
>>>>>>> 0c00536c28e3afc43755db64debd0098a64102e0

```{r counties-barplot, echo=FALSE, warning=FALSE, message=FALSE}

counties_current_barplot <- function(state_plot, variable) {
  us_counties_current %>%
...
p1 <- counties_current_barplot("California", quo(cases))
p2 <- counties_current_barplot("California", quo(deaths))
...
lag_day <- lag_days(by_country, countries) %>% select(country, cases) %>% deframe()
...
by_country %>%
  ...
  ggplot(aes(x=lag_date, y=cases, group=country)) +
...
```{r by-county, echo=FALSE, warning=FALSE, message=FALSE}

plot_counties <- function(plot_country, plot_state, variable, topn=3) {
  complete %>%
...
plot_counties("US", "Michigan", quo(deaths))
plot_counties("US", "Michigan", quo(cases))

plot_counties("US", "California", quo(deaths))
plot_counties("US", "California", quo(cases))
