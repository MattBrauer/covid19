...
p1 <- csse_by_country %>%
  ...
  timeplot(quo(new_cases), show_lines = FALSE, show_points = TRUE, show_average = FALSE, lag_percentile = 0)
# l1 <- csse_by_country %>%
#   filter(country %in% countries) %>%
#   group_by(country) %>%
#   daily_from_cumulative() %>%
#   rolling_average() %>%
#   timeplot(quo(avg_new_cases), show_lines = TRUE, show_points = FALSE, lag_percentile = 0)
p2 <- csse_by_country %>%
  filter(country %in% countries) %>%
  group_by(country) %>%
  daily_from_cumulative() %>%
  timeplot(quo(new_deaths), show_lines = FALSE, show_points = TRUE, show_average = FALSE, lag_percentile = 0)
grid.arrange(p1, p2, nrow = 1)
