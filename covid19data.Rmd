...
plot_counties <- function(plot_country, plot_state, parameter, topn=3) {
  complete %>%
    filter(country==plot_country, state==plot_state, !is.na(Admin2)) %>%
    group_by(Admin2) %>%
  { ggplot(.) +
    geom_line(aes(x=date, y=(!!parameter), color=Admin2)) +
    geom_text(data = . %>%
                filter(date == max(date)) %>%
                ungroup() %>%
                top_n(topn, (!! parameter)),
              aes(label = Admin2, color = Admin2, x = date, y = (!! parameter)),
              hjust = "right",
              vjust = "bottom") +
      theme_minimal() +
      theme(legend.position = "none") +
      ggtitle(paste0(plot_state, " ", quo_name(parameter), " by county"))
    }
}
