...
```{r ihme-vs-cubic, echo=FALSE, warning=FALSE, message=FALSE}
csse_by_country %>%
  filter(country == "US") %>%
...
```{r featured-plots-0, echo=FALSE, warning=FALSE, message=FALSE}
...
## Daily new cases, comparing US to EU and Brazil

```{r featured-plots-1, echo=FALSE, warning=FALSE, message=FALSE}

csse_by_country %>%
  mutate(region = if_else(country %in% eu, "EU", country)) %>%
  group_by(region, date) %>%
  summarize_if(is.numeric, sum, na.rm = TRUE) %>%
  daily_from_cumulative() %>%
  rolling_average(window = 3) %>%
  filter(region %in% c("EU", "US", "Brazil")) %>%
  timeplot(quo(new_cases), show_lines = FALSE, show_points = TRUE, show_average = TRUE, topn = 4)
...
csse_by_country %>%
  ...
  rolling_average(window = 3) %>%
  filter(region %in% c("EU", "US", "Brazil")) %>%
  timeplot(quo(new_deaths), show_lines = FALSE, show_points = TRUE, show_average = TRUE, topn = 4)


```


```{r featured-grouped-states, echo=FALSE, warning=FALSE, message=FALSE}

groups <- rep("rest", length(us_state_pop$NAME))
groups[us_state_pop$NAME %in% c("California")] <- "CA"
groups[us_state_pop$NAME %in% c("Arizona", "Florida", "Texas")] <- "AZ-FL-TX"
groups[us_state_pop$NAME %in% c("New York", "New Jersey")] <- "NY-NJ"
groups[us_state_pop$NAME %in% c("United States",
                                "Northeast Region",
                                "Midwest Region",
                                "South Region",
                                "West Region")] <- "aggregate"
names(groups) <- us_state_pop$NAME

pops <- tbl_df(list(pop=us_state_pop$pop,
                  state=us_state_pop$NAME,
                  wave=groups)) %>%
  group_by(wave) %>%
...
csse_us_states %>%
  daily_from_cumulative() %>%
  mutate(wave = groups[state]) %>%
  group_by(wave, date) %>%
  summarize_if(is_double, sum, na.rm=TRUE) %>%
  group_by(wave) %>%
  rolling_average(window = 3) %>%
  timeplot(quo(new_cases), show_lines = FALSE, show_points = TRUE, show_average = TRUE, hide_legend = FALSE, topn = 0)
...
csse_us_states %>%
  ...
  rolling_average(window = 3) %>%
  left_join(pops) %>%
  mutate_at(vars(-wave, -date, -pop), funs(. / (pop/1e6))) %>%
  timeplot(quo(new_cases), show_lines = FALSE, show_points = TRUE, show_average = TRUE, hide_legend = FALSE, topn = 0)
...
```{r featured-plots-2, echo=FALSE, warning=FALSE, message=FALSE}
...
state_daily_stats %>%
  filter(state %in% c("Illinois", "Nevada", "Maryland", "California")) %>%
  group_by(state) %>%
  rolling_average() %>%
  { ggplot(., aes(x=date, y=avg_new_cases)) +
      facet_wrap(state ~ ., scales = "free_y", ncol = 2) +
...
```{r featured-plots-3, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
state_daily_stats %>%
  filter(state %in% c("Arizona", "Illinois", "Texas", "Utah")) %>%
  group_by(state) %>%
  rolling_average() %>%
  { ggplot(., aes(x=date, y=avg_new_deaths)) +
      facet_wrap(state ~ ., scales = "free_y", ncol = 2) +
...
dataset %>% 
  timeplot(quo(new_cases), hide_legend = FALSE, show_lines = TRUE, show_points = TRUE, show_average = TRUE, lag_percentile = 0)

dataset %>% 
  timeplot(quo(new_deaths), hide_legend = FALSE, show_lines = TRUE, show_points = TRUE, show_average = TRUE, lag_percentile = 0)
...
```{r exp-fit-table, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}
exp_fits %>%
  select(-data) %>%
...
focal_states <- c("Arizona", "Arkansas")
...
p1 <- county_exp_fits %>%
  county_map(quo(max_est), log_scale = FALSE)
p2 <- county_exp_fits %>%
  county_map(quo(last_est), log_scale = FALSE)
...
p4 <- csse_us_states %>%
  filter(state %in% focal_states, cases > 9) %>%
  mutate(cases = log(cases + 1)) %>%
...
```{r state-max-exp-fit-map, echo=FALSE, warning=FALSE, message=FALSE}
exp_fits %>%
  select(-data) %>%
  ungroup() %>%
  state_map(quo(max_est))
```
...
```{r state-last-exp-fit-map, echo=FALSE, warning=FALSE, message=FALSE}
exp_fits %>%
  select(-data) %>%
  ungroup() %>%
  state_map(quo(last_est))
```
