...
# refresh data from JHU github repo
...
bounds_cases <- list(country=c("China","New York","Italy","Spain",
                         "California","Washington", "US"),
               min_date=as.Date(c("2020-01-22", "2020-03-08", rep("2020-03-01", 5))),
               max_date=as.Date(c("2020-02-07", "2020-06-01",
                                    "2020-03-22", rep("2020-06-01", 4))))
bounds_deaths <- list(country=c("China","New York","Italy","Spain",
                         "California","Washington", "US"),
               min_date=as.Date(c("2020-01-22", "2020-03-13", rep("2020-03-01", 5))),
               max_date=as.Date(c("2020-02-07", "2020-03-24", rep("2020-06-01", 5))))

exp_phase_fit <- function(dataset, bounds, variable) {
  exp_start <- as.tibble(bounds) %>% select(country, min_date) %>% deframe()
  exp_end <- as.tibble(bounds) %>% select(country, max_date) %>% deframe()
  dataset %>%
