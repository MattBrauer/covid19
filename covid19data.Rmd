...
lag_days <- c(0, 33, 44, 44, 53, 44, 53)
...
# time series files using the old data model, good through March 22 and 
...
data_model_start_dates <- as.Date(c("01-22-2020", "03-23-2020"), "%m-%d-%Y")

data_models <- Vectorize(function(obs_date, start_dates) {
  end_dates <- lead(c(start_dates,today())-days(1))[1:length(start_dates)]
...
report_models <- data_model(report_dates, data_model_start_dates)
report_sets <- as_tibble(list(file = report_files,
                              date = report_dates,
                              model = report_models))

report_data <- report_sets %>%
  split(.$model) %>%
  purrr::map(~ .x %>%
               dplyr::pull(file) %>%
               map_dfr(read_csv, .id="file", col_types=cols(
...
complete <- rbind(complete,
  ...
  rename("state"=Province_State,
         ...
         "recovered"=Recovered,
         "Long"=Long_) %>%
  select(state, country, Lat, Long, date, confirmed, recovered, deaths)
)
```


```{r load-new-data-model}
...
by_country %>%
  ...
  mutate("day" = date - min(date),
         "lag_date" = date - lag_days[country],
         "lag_days" = lag_date - min(date)) %>%
  ggplot(aes(x=lag_days, y=deaths, group=country)) +
  geom_point(aes(color=country)) +
  geom_line(aes(color=country)) +
  xlab("Day of pandemic") +
  ylab("Deaths") +
...
by_country %>%
  filter(country %in% countries) %>%
  mutate("day" = date - min(date),
         "lag_date" = date - days(lag_days[country] - lag_days["Italy"]),
         "lag_days" = lag_date - min(date)) %>%
  ggplot(aes(x=lag_days, y=confirmed, group=country)) +
  geom_point(aes(color=country)) +
  geom_line(aes(color=country)) +
...
by_country_state %>%
  ...
  mutate("day" = date - min(date),
         ...
         "lag_days" = lag_date - min(date)) %>%
  rename("region" = country) %>%
  ggplot(aes(x=lag_days, y=deaths, group=region)) +
  geom_point(aes(color=region)) +
  geom_line(aes(color=region)) +
  xlab("Day of pandemic") +
  ylab("Deaths") +
...
by_country_state %>%
  filter(country %in% exp_phase_countries) %>%
  mutate("day" = date - lag_days[country], "log_confirmed" = log10(confirmed + 1)) %>%
